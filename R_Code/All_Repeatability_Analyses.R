...
# Scrambling birds according to their dominance status.
...
Third_Bound <- Third_Bound %>%
  ...
  mutate(Bird_Present = factor(Bird_Present), Group = factor(Group)) %>% 
  mutate(Bird.ID = ifelse(Bird.ID == "YAO1", "YAO2", 
    ifelse(Bird.ID == "YAO2", "YAO1", Bird.ID)
    )
  ) %>% 
  mutate(Bird.ID = factor(Bird.ID)) 
  
  # Note that enumeration of YA0 birds is corrected to match dominance data (although both are subordinate, just in different pens)

Third_Bound %>% group_by(Bird.ID, Pen) %>% 
  filter(!is.na(Bird.ID)) %>% 
  summarise(Count = n()) %>% 
  as.data.frame(.)

# Good. Adding in dominance status

Stats_Base <- read.csv("/home/joshk/git_repositories/BCCH_Dominance/Data/Rank_Summaries.csv")
Status_Clean <- as.data.frame(Stats_Base %>% 
  dplyr::select(Bird.ID, Pen, "Status" = RSE.Status)) %>% 
  mutate(Bird.ID = 
    ifelse(Bird.ID == "BdAO", "BdAO1", 
      ifelse(Bird.ID == "YAO", "YAO1", 
        ifelse(Bird.ID == "BdABl", "BdABl1", Bird.ID)
      )
    )
  )

TB_Dom <- left_join(Third_Bound, Status_Clean, by = c("Bird.ID", "Pen"))

# Splitting by group and social status

TB_Dom_Split <- split(TB_Dom, f = TB_Dom$Group)

for (i in 1:length(TB_Dom_Split)) {
  TB_Dom_Split[[i]] <- split(TB_Dom_Split[[i]], f = TB_Dom_Split[[i]]$Bird_Present)
}

for (i in 1:length(TB_Dom_Split)) {
  TB_Dom_Split[[i]][[2]] <- split(TB_Dom_Split[[i]][[2]], f = TB_Dom_Split[[i]][[2]]$Status)
}

# Setting seed and randomly scrambling bird identities among data points within each status grouping
...
for (k in 1:2) {
  for (j in 1:2) {
    Bird_Scramble <- c()
    for (i in 1:nrow(TB_Dom_Split[[k]][[2]][[j]])) {
      Bird_Scramble[i] <- paste(sample(unique(TB_Dom_Split[[k]][[2]][[j]]$Bird.ID), 1))
    }
    TB_Dom_Split[[k]][[2]][[j]]$Bird_Scramble <- factor(Bird_Scramble)
  }
}  

# Re-joining and re-arranging data

for (i in 1:2) {
  TB_Dom_Split[[i]][[2]] <- bind_rows(TB_Dom_Split[[i]][[2]])
}
  
for (i in 1:2){
  TB_Dom_Split[[i]] <- bind_rows(TB_Dom_Split[[i]])
}

TB_Dom_Bound <- bind_rows(TB_Dom_Split) %>% 
  arrange(RowID) %>%
  dplyr::select(-c(Bird_Present, Group, RowID)) %>%
  mutate(Status = factor(Status),
    Bird_Scramble = factor(Bird_Scramble)
  )

if (nrow(Third_Bound) == nrow(TB_Dom_Bound)){
  print("No data lost in bind")
} else {
...
#write.csv(TB_Dom_Bound, "/home/joshk/git_repositories/BCCH_IndVar/Data/BCCH_Dom_Scramble.csv", row.names = FALSE)
TB_Dom_Bound = read.csv("/home/joshk/git_repositories/BCCH_IndVar/Data/BCCH_Dom_Scramble.csv")

###############################################################################
## Now, checking differences in status between urban and rural birds ##
###############################################################################

Cond_Bound = read.csv("/home/joshk/git_repositories/BCCH_Dominance/Data/Condition_Final.csv")
Cond_Bound$Status_Binomial = as.numeric(Cond_Bound$RSE.Status) - 1

# Correcting oddities in labels
Cond_Bound$Sex = as.character(Cond_Bound$Sex)
Cond_Bound$Sex[c(which(Cond_Bound$Bird.ID == "YAO1"))] = "Male"
Cond_Bound$Sex[c(which(Cond_Bound$Bird.ID == "YAO2"))] = "Female"
Cond_Bound$Capture.Locale = as.character(Cond_Bound$Capture.Locale)
Cond_Bound$Capture.Locale[c(which(Cond_Bound$Bird.ID == "YAO1"))] = "Brantford"
Cond_Bound = Cond_Bound %>% 
    mutate(Sex = factor(Sex),
        Capture.Locale = factor(Capture.Locale))

prior_status <- c(
  set_prior("normal(0, 2.5)", class = "b", coef = "SexMale"),
  set_prior("normal(0, 2.5)", class = "b", coef = "Locale.TypeUrban")
)

Status_Mod = brm(Status_Binomial ~ Locale.Type + Sex + 
  (1|Capture.Locale),  
  data = subset(Cond_Bound, Treatment == "Stress"),
   cores = 4, chains = 4,
  seed = 100, refresh = 0, family = "bernoulli",
  iter = 10000, warmup = 1000, thin = 10,
  control = list(adapt_delta = 0.98, max_treedepth = 14),
  prior = prior_status
  )

pp_check(Status_Mod, nsamples = 500) # Odd, but fair. Skipping residual check for now.
summary(Status_Mod) # No detected differences between statuses or sexes

Status_Plot = subset(Cond_Bound, Treatment == "Stress") %>%
  dplyr::select(
    Sex, Locale.Type, Capture.Locale
  ) %>%
  na.omit(.) %>%
  add_predicted_draws(Status_Mod) %>%
  group_by(
    Locale.Type
  ) %>% 
  summarise("Fit" = 1- mean(.prediction),
    "UCL" = 1 - quantile(.prediction, 0.975, type = 8),
    "LCL" = 1 - quantile(.prediction, 0.025, type = 8)
  ) %>%
  ggplot(aes(x = Locale.Type, y = Fit, fill = Locale.Type)) + 
    geom_errorbar(aes(x = Locale.Type, ymin = LCL, ymax = UCL),
      colour = "black", size = 1, width = 0.4) + 
    geom_point(size = 5, pch = 21, colour = "black") + 
    xlab("Ecotype") + ylab("Social Status") + 
    scale_fill_manual(values = c("slateblue", "wheat")) + 
    theme_bw() + my.theme + theme(legend.position = "none") + 
    ylim(c(-0.25,1.25))

Status_Plot

ggsave("/home/joshk/git_repositories/BCCH_IndVar/Figures/Supp_Status_by_Ecotype.jpeg",
  Status_Plot,
  height = 6.0, width = 7.0, dpi = 800
)

# Running raw plots on both binomial status and rank

Cond_Bound %>% 
  filter(Treatment == "Stress") %>% 
  ggplot(aes(x = Locale.Type, y = RSE.Status, fill = Locale.Type)) + 
   stat_summary(geom = "errorbar", fun.data = "mean_cl_boot", 
      colour = "black", size = 1, width = 0.4) + 
    stat_summary(geom = "point", fun = "mean",   
      size = 5, pch = 21, colour = "black") + 
    xlab("Ecotype") + ylab("Social Status") + 
    scale_fill_manual(values = c("slateblue", "wheat")) + 
    theme_bw() + my.theme + theme(legend.position = "none")
...
Cond_Bound %>% 
  filter(Treatment == "Stress") %>% 
  ggplot(aes(x = Locale.Type, y = RSE.Rank, fill = Locale.Type)) + 
   stat_summary(geom = "errorbar", fun.data = "mean_cl_boot", 
      colour = "black", size = 1, width = 0.4) + 
    stat_summary(geom = "point", fun = "mean",   
      size = 5, pch = 21, colour = "black") + 
...
# Raw data suggest a trend toward rural birds being dominant when sex is unaccounted.
## Checking differences in condition between urban and rural birds

CB_NaFree = as.data.frame(Cond_Bound %>% 
    dplyr::select(Mass.x, Mass.y, Treatment, RSE.Status, Bird.ID, Pen, Sex, Locale.Type) %>% 
    na.omit(.)) %>% mutate(Delta = Mass.x - Mass.y)
CB_NaFree$RSE.Status = factor(CB_NaFree$RSE.Status)
CB_NaFree$Treatment = factor(CB_NaFree$Treatment)
CB_NaFree$Locale.Type = factor(CB_NaFree$Locale.Type)

prior_delta <- c(
  set_prior("normal(0, 3.5)", class = "b", coef = "SexMale"),
  set_prior("normal(0, 3.5)", class = "b", coef = "Locale.TypeUrban"),
  set_prior("normal(0, 3.5)", class = "b", coef = "TreatmentStress")
)

Diff_Mod = brm(Delta ~ Treatment*Locale.Type + Sex + (1|Bird.ID),
  sigma ~ Treatment, data = CB_NaFree, 
  cores = 4, chains = 4, seed = 100, refresh = 0, 
  family = "gaussian", iter = 10000, warmup = 1000, 
  thin = 10, control = list(adapt_delta = 0.98, max_treedepth = 14),
  prior = prior_delta)

pp_check(Diff_Mod) # Fair. Again, passing residual checks.
plot(Diff_Mod)
summary(Diff_Mod) # No effects of ecotype on change in mass.

# Checking at specific timepoints

Beginning = brm(Mass.y ~ Treatment*Locale.Type + Sex + (1|Bird.ID),
  sigma ~ Treatment, data = CB_NaFree, 
  cores = 4, chains = 4, seed = 100, refresh = 0, 
  family = "gaussian", iter = 10000, warmup = 1000, 
  thin = 10, control = list(adapt_delta = 0.98, max_treedepth = 14),
...
End = brm(Mass.x ~ Treatment*Locale.Type + Sex + (1|Bird.ID),
  sigma ~ Treatment, data = CB_NaFree, 
  cores = 4, chains = 4, seed = 100, refresh = 0, 
  family = "gaussian", iter = 10000, warmup = 1000, 
  thin = 10, control = list(adapt_delta = 0.98, max_treedepth = 14),
...
summary(Beginning) # Only effect of sex (males being larger)
summary(End) # And again, only effect of sex

# Plotting all mass results.

Plot_Dat = CB_NaFree %>% group_by(Locale.Type) %>% 
  summarise(Start_Mass = mean(Mass.y),
    Start_LCL = quantile(Mass.y, 0.025, type = 8),
    Start_UCL = quantile(Mass.y, 0.975, type = 8),
    End_Mass = mean(Mass.x),
    End_LCL = quantile(Mass.y, 0.025, type = 8),
    End_UCL = quantile(Mass.y, 0.975, type = 8),
    Delta_Mean = mean(Delta),
    Delta_LCL = quantile(Delta, 0.025, type = 8),
    Delta_UCL = quantile(Delta, 0.975, type = 8)
    )

Ecotype_Plot_End = ggplot(Plot_Dat, 
  aes(x = Locale.Type, y = End_Mass, fill = Locale.Type)) + 
  geom_errorbar(aes(x = Locale.Type, ymin = End_LCL, ymax = End_UCL),
    size = 1, colour = "black", width = 0.4) +
  geom_point(size = 5, pch = 21, colour = "black") + 
  scale_fill_manual(values = c("slateblue", "wheat")) + 
  theme_bw() + my.theme + theme(legend.position = "none") + 
  xlab("Ecotype") + ylab("Mass (g)") + ylim(c(8, 13))    

Ecotype_Plot_End

ggsave("/home/joshk/git_repositories/BCCH_IndVar/Figures/Supp_Mass_by_Ecotype_End.jpeg",
  Ecotype_Plot_End,
  height = 6.0, width = 7.0, dpi = 800
)

Ecotype_Plot_Delta = ggplot(Plot_Dat, 
  aes(x = Locale.Type, y = Delta_Mean, fill = Locale.Type)) + 
  geom_errorbar(aes(x = Locale.Type, ymin = Delta_LCL, ymax = Delta_UCL),
    size = 1, colour = "black", width = 0.4) +
  geom_point(size = 5, pch = 21, colour = "black") + 
  scale_fill_manual(values = c("slateblue", "wheat")) + 
  theme_bw() + my.theme + theme(legend.position = "none") + 
  xlab("Ecotype") + ylab(" Change in Mass (g)") + ylim(c(-1.5,1.5)) 

Ecotype_Plot_Delta

ggsave("/home/joshk/git_repositories/BCCH_IndVar/Figures/Supp_Mass_by_Ecotype_Delta.jpeg",
  Ecotype_Plot_Delta,
  height = 6.0, width = 7.0, dpi = 800
)
...
Beginning_Condition = brm(Condition.y ~ Treatment*Locale.Type + Sex + (1|Bird.ID),
  sigma ~ Treatment, data = Cond_Bound,
  cores = 4, chains = 4, seed = 100, refresh = 0, 
  family = "gaussian", iter = 10000, warmup = 1000, 
  thin = 10, control = list(adapt_delta = 0.98, max_treedepth = 14),
  prior = prior_delta)

End_Condition = brm(Condition.x ~ Treatment*Locale.Type + Sex + (1|Bird.ID),
  sigma ~ Treatment, data = Cond_Bound, 
  cores = 4, chains = 4, seed = 100, refresh = 0, 
  family = "gaussian", iter = 10000, warmup = 1000, 
  thin = 10, control = list(adapt_delta = 0.98, max_treedepth = 14),
...
pp_check(Beginning_Condition)
pp_check(End_Condition)

summary(Beginning_Condition)
summary(End_Condition) # Subtle effect of males being in better condition at the end of trials. Plotting.

Plot_Dat = Cond_Bound %>% group_by(Locale.Type) %>% 
  summarise(Condition = mean(Condition.x, na.rm = T),
    Condition_LCL = quantile(Condition.x, 0.025, type = 8, na.rm = T),
    Condition_UCL = quantile(Condition.x, 0.975, type = 8, na.rm = T)
    )

Ecotype_Plot_Condition = ggplot(Plot_Dat, 
  aes(x = Locale.Type, y = Condition, fill = Locale.Type)) + 
  geom_errorbar(aes(x = Locale.Type, ymin = Condition_LCL, ymax = Condition_UCL),
    size = 1, colour = "black", width = 0.4) +
  geom_point(size = 5, pch = 21, colour = "black") + 
  scale_fill_manual(values = c("slateblue", "wheat")) + 
  theme_bw() + my.theme + theme(legend.position = "none") + 
  xlab("Ecotype") + ylab("Condition (0-1)")

Ecotype_Plot_Condition

ggsave("/home/joshk/git_repositories/BCCH_IndVar/Figures/Supp_Condition_by_Ecotype.jpeg",
  Ecotype_Plot_Condition,
  height = 6.0, width = 7.0, dpi = 800
)

# Checking effects of condition on control and stress-exposed slopes for birds of each ecotype 

{
  All_Slopes <- CC_Full %>%
  spread_draws(s_ss.Amb.TempBird.IDTreatmentStress_1[ID],
  s_ss.Amb.TempBird.ID_1[ID]) %>%
  rename("S_Slope" = s_ss.Amb.TempBird.IDTreatmentStress_1,
  "C_Slope" = s_ss.Amb.TempBird.ID_1) %>%
  as.data.frame(.)
  ...
  All_Collapsed <- left_join(All_Slopes, Birds, by = c("ID")) %>%
  dplyr::select(-ID) %>%
  group_by(Bird_ID) %>%
  summarise("C_Slope" = mean(C_Slope),
...
Locale <- c()
  ...
  for (i in 1:nrow(All_Collapsed)) {
  if (is.na(All_Collapsed$Bird_ID[i])) {
    Locale[i] <- NA
    U_R[i] <- NA
  } else if (All_Collapsed$Bird_ID[i] == "BdABd") {
    Locale[i] <- "Corwhin"
    U_R[i] <- "Rural"
  } else if (All_Collapsed$Bird_ID[i] == "BdAO1") {
    Locale[i] <- "Erin"
    U_R[i] <- "Rural"
  } else if (All_Collapsed$Bird_ID[i] == "ABlBl") {
    Locale[i] <- "Ruthven.Park"
    U_R[i] <- "Rural"
  } else if (All_Collapsed$Bird_ID[i] == "AYY") {
    Locale[i] <- "Brantford"
    U_R[i] <- "Urban"
  } else if (All_Collapsed$Bird_ID[i] == "YAO1") {
    Locale[i] <- "Corwhin"
    U_R[i] <- "Rural"
  } else if (All_Collapsed$Bird_ID[i] == "OOA") {
    Locale[i] <- "Ruthven.Park"
    U_R[i] <- "Rural"
  } else if (All_Collapsed$Bird_ID[i] == "BdABl1") {
    Locale[i] <- "Ruthven.Park"
    U_R[i] <- "Rural"
  } else if (All_Collapsed$Bird_ID[i] == "YAO2") {
    Locale[i] <- "Brantford"
    U_R[i] <- "Urban"
  } else if (All_Collapsed$Bird_ID[i] == "ABlO") {
    Locale[i] <- "Brantford"
    U_R[i] <- "Urban"
  } else if (All_Collapsed$Bird_ID[i] == "ABdBd") {
    Locale[i] <- "Guelph"
    U_R[i] <- "Urban"
  } else if (All_Collapsed$Bird_ID[i] == "YAR") {
    Locale[i] <- "Cambridge"
    U_R[i] <- "Urban"
  } else if (All_Collapsed$Bird_ID[i] == "RAR") {
    Locale[i] <- "Corwhin"
    U_R[i] <- "Rural"
  } else if (All_Collapsed$Bird_ID[i] == "ABlR") {
    Locale[i] <- "Brantford"
    U_R[i] <- "Urban"
  } else if (All_Collapsed$Bird_ID[i] == "ARO") {
    Locale[i] <- "Corwhin"
    U_R[i] <- "Rural"
  } else if (All_Collapsed$Bird_ID[i] == "BdABl2") {
    Locale[i] <- "Corwhin"
    U_R[i] <- "Rural"
  } else if (All_Collapsed$Bird_ID[i] == "BlAR") {
    Locale[i] <- "Corwhin"
    U_R[i] <- "Rural"
  } else if (All_Collapsed$Bird_ID[i] == "BdAO2") {
    Locale[i] <- "Guelph"
    U_R[i] <- "Urban"
  } else if (All_Collapsed$Bird_ID[i] == "AOR") {
    Locale[i] <- "Guelph"
    U_R[i] <- "Urban"
  } else if (All_Collapsed$Bird_ID[i] == "YAY") {
    Locale[i] <- "Guelph"
    U_R[i] <- "Urban"
  } else if (All_Collapsed$Bird_ID[i] == "AYBd") {
    Locale[i] <- "Erin"
    U_R[i] <- "Rural"
  } else {
    Locale[i] <- NA
...
All_Collapsed$U_R <- factor(U_R)
All_Collapsed$Locale <- factor(Locale)

All_Long <- pivot_longer(All_Collapsed, cols = c("C_Slope", "S_Slope"), names_to = "Treatment", values_to = "Slope") %>% mutate("Treatment" = ifelse(Treatment == "C_Slope", "Rest", "Stress"))

# Adjusting individual IDs to match across files, then binding 

All_Bound <- left_join((All_Long %>%
    mutate(Bird_ID = ifelse(Bird_ID == "YAO2", "YAO1", Bird_ID))
    ), (Cond_Bound %>% 
  select("Bird_ID" = Bird.ID, Sex, "End_Mass" = Mass.x, "Start_Mass" = Mass.y,
    "End_Condition" = Condition.x, "Start_Condition" = Condition.y, 
    "Status" = RSE.Status, Treatment)
  ), by = c("Bird_ID", "Treatment")) %>% as.data.frame(.)

ggplot(All_Bound, aes(x = End_Mass, y = Slope, fill = Treatment, linetype = Treatment)) + 
  geom_point(size = 4, pch = 21, colour = "black") + 
  geom_smooth(method = "lm", size = 1, colour = "black", se = FALSE) + 
  scale_fill_manual(values = c("slateblue", "wheat")) + 
  theme_bw() + my.theme + xlab("End Mass (g)") + ylab("Slope") + 
  facet_grid(~U_R)

# Quite interesting. Trying with condition

ggplot(All_Bound, aes(x = End_Condition, y = Slope, fill = Treatment, 
  linetype = Treatment)) + 
  geom_point(size = 4, pch = 21, colour = "black") + 
  geom_smooth(method = "lm", size = 1, colour = "black", se = FALSE) + 
  scale_fill_manual(values = c("slateblue", "wheat")) + 
...
ggplot(All_Bound, aes(x = End_Mass - Start_Mass, y = Slope, fill = Treatment, 
  linetype = Treatment)) + 
  geom_point(size = 4, pch = 21, colour = "black") + 
  geom_smooth(method = "lm", size = 1, colour = "black", se = FALSE) + 
  scale_fill_manual(values = c("slateblue", "wheat")) + 
...
# Running quick model with only stress exposed data

Stress_Only <- subset(All_Bound, Treatment == "Stress")

prior_slopes <- c(
    set_prior("normal(0, 3.5)", class = "b", coef = "U_RUrban"),
    set_prior("normal(0, 3.5)", class = "b", coef = "End_Mass")
)

Slopes_Mass_Mod = brm(Slope ~ U_R*End_Mass + (1|Locale), 
    data = Stress_Only, cores = 4, chains = 4, seed = 100, refresh = 0, 
    family = "gaussian", iter = 10000, warmup = 1000, 
    thin = 10, control = list(adapt_delta = 0.98, max_treedepth = 14),
    prior = prior_slopes)

pp_check(Slopes_Mass_Mod, nsamples = 500) # Acceptable, but odd nuiances in model. Follow up on this.
pp_check(Slopes_Mass_Mod, nsamples = 500, stat = "median")

plot(Slopes_Mass_Mod)
summary(Slopes_Mass_Mod) 

emmip(Slopes_Mass_Mod, U_R ~ End_Mass, 
  data = Stress_Only, CIs = TRUE, 
  at = list(End_Mass = seq(min(Stress_Only$End_Mass, na.rm = T),
  max(Stress_Only$End_Mass, na.rm = T), by = 0.5))
)

# Checking fit

yrep <- posterior_predict(Slopes_Mass_Mod, nsamples = 100)
...
# Fit rather poor for urban birds. Checking residuals

Slopes_Mass_Mod$data %>%
  dplyr::select(
    Slope, U_R, End_Mass, Locale
  ) %>%
  na.omit(.) %>%
  add_predicted_draws(Slopes_Mass_Mod) %>%
  group_by(
    Slope, U_R, End_Mass, Locale
  ) %>%
  summarise("Fit" = mean(.prediction)) %>%
  add_predicted_draws(Slopes_Mass_Mod) %>%
  summarise(
    p_residual = mean(.prediction < Slope),
    z_residual = qnorm(p_residual)
  ) %>%
  ggplot(aes(x = Fit, y = z_residual)) +
  geom_point(size = 2, pch = 21, colour = "black", fill = "cornflowerblue") + 
...
Slopes_Mass_Mod$data %>%
  ...
  na.omit(.) %>%
  add_residual_draws(Slopes_Mass_Mod) %>%
  ggplot(aes(x = .residual)) +
  geom_density()

# Checking patterns across predictors.

{
  p1 <- Slopes_Mass_Mod$data %>%
    dplyr::select(
        Slope, U_R, End_Mass, Locale
    ) %>%
    na.omit(.) %>%
    ...
    ggplot(aes(x = U_R, y = z_residual, fill = U_R)) +
    geom_boxplot() +
    scale_fill_viridis_d(begin = 0.3, end = 0.6)

  p2 <- Slopes_Mass_Mod$data %>%
    dplyr::select(
        Slope, U_R, End_Mass, Locale
    ) %>%
    na.omit(.) %>%
    ...
    ggplot(aes(
      x = End_Mass, y = z_residual,
      fill = U_R, linetype = U_R
    )) +
    geom_point(size = 2, pch = 21, colour = "black") +
    geom_smooth(method = "loess", colour = "black") +
    scale_fill_viridis_d(begin = 0.3, end = 0.6)
...
# Clear heteroskedasticity by ecotype. Re-running model with variance estimated seperately per group.

Slopes_Mass_Adj = brm(bf(Slope ~ U_R*End_Mass + (1|Locale), 
    sigma ~ U_R),
    data = Stress_Only, cores = 4, chains = 4, seed = 100, refresh = 0, 
    family = "gaussian", iter = 10000, warmup = 1000, 
    thin = 10, control = list(adapt_delta = 0.98, max_treedepth = 14),
...
{
  p1_adj <- Slopes_Mass_Adj$data %>%
    dplyr::select(
        Slope, U_R, End_Mass, Locale
    ) %>%
    na.omit(.) %>%
    add_predicted_draws(Slopes_Mass_Adj) %>%
    summarise(
      p_residual = mean(.prediction < Slope),
      z_residual = qnorm(p_residual)
    ) %>%
...
dev.new()
grid.arrange(p1_adj, p2_adj, nrow = 1)

# Slightly better. Checking fitted values by ecotype

yrep <- posterior_predict(Slopes_Mass_Adj, nsamples = 100)
ppc_scatter_avg_grouped(y, yrep, group = Group)

# Still quite poor. 

loo_R2(Slopes_Mass_Mod); loo_R2(Slopes_Mass_Adj)

# Interesting. Variance structure actually reduces loo R2. Perhaps data is simply too messy.





## Checking effects of ecotype on chronic and acute slopes after accounting for differences in status. Beginning with chronic slopes
...
Collapsed$U_R <- factor(U_R)
Collapsed$Locale <- factor(Locale)
Status_Clean$Pen = as.character(Status_Clean$Pen)
Status_Clean$Pen[c(which(Status_Clean$Bird.ID == "YAO1"))] = "NE"
Status_Clean$Pen[c(which(Status_Clean$Bird.ID == "YAO2"))] = "NW"
Status_Clean$Pen = factor(Status_Clean$Pen)
Status_Clean = Status_Clean %>% rename("Bird_ID" = Bird.ID)
Collapsed = left_join(Collapsed, Status_Clean, by = c("Bird_ID"))
...
Collapsed <- left_join(Collapsed, Orders, by = c("Bird_ID")) %>%
  mutate(Order = factor(Order), U_R = factor(U_R))
Collapsed$Status = as.factor(Collapsed$Status)

# Running model

prior_slopes <- c(
  set_prior("normal(0, 3.5)", class = "b", coef = "U_RUrban"),
  set_prior("normal(0, 3.5)", class = "b", coef = "Status2")
)

Slopes_Mod_Chronic = brm(Slope ~ U_R + Status + (1|Locale), 
  data = Collapsed, cores = 4, chains = 4, seed = 100, refresh = 0, 
  family = "gaussian", iter = 10000, warmup = 1000, 
  thin = 10, control = list(adapt_delta = 0.98, max_treedepth = 14),
  prior = prior_slopes)

pp_check(Slopes_Mod_Chronic, nsamples = 250) # Again, double peaks which may be a consequence of differing residual variance between urban and rural birds.
plot(Slopes_Mod_Chronic) # Status trending towards significance, as expected.
summary(Slopes_Mod_Chronic)

Collapsed %>% ggplot(aes(x = Status, y = Slope, fill = Status)) + 
    stat_summary(geom = "errorbar", fun.data = "mean_cl_boot",
        size = 1, colour = "black", width = 0.4) + 
    stat_summary(geom = "point", fun = "mean", pch = 21, size = 5,
        colour = "black") + 
    scale_fill_manual(values = c("slateblue", "lightseagreen")) + 
    theme_bw()        

### Reversed from our previous study?
...
Acute_Collapsed = left_join(Acute_Collapsed, Status_Clean, by = c("Bird_ID"))
...
Acute_Collapsed$RSE.Status = as.factor(Acute_Collapsed$RSE.Status)

Slopes_Mod_Acute = brm(Slope ~ U_R + RSE.Status, 
  data = Acute_Collapsed, cores = 4, chains = 4, seed = 100, refresh = 0, 
  family = "gaussian", iter = 10000, warmup = 1000, 
  thin = 10, control = list(adapt_delta = 0.98, max_treedepth = 14),
  prior = prior_slopes)

pp_check(Slopes_Mod_Acute)
plot(Slopes_Mod_Acute)
summary(Slopes_Mod_Acute) # Absolutely no effect of status or ecotype.