...
file = "/home/joshk/git_repositories/SIH_Repeatability/brms_outputs/Acute_Regular_DomS.Rds")

# Assessing posterior distributions

pp_check(Acute_Full_DS, nsamples = 100) # Great - checking patterns across treatment types.

yrep_Acute <- posterior_predict(Acute_Full_DS, nsamples = 100)
...
# Good. Heterogeneity, although such is expected given effects of stress-exposure on eye region temperature across ambient temperature and time. Checking fitteds by response across treatments

ppc_scatter_avg_grouped(Acute_y, yrep_Acute, group = Acute_Group)

# No apparent biases. Assessing MCMC autocorrelation, effective sample sizes to sample size ratios, and Rhats.

stan_ac(Acute_Full_DS$fit) # No concern
mcmc_neff(neff_ratio(Acute_Full_DS), size = 2) # > 0.7
mcmc_rhat(rhat(Acute_Full_DS)) # Quite close to one. Checking for abberant patterns in residuals.
...
Acute_Full_DS$data %>%
  ...
  na.omit(.) %>%
  add_predicted_draws(Acute_Full_DS) %>%
  group_by(
    Maximum.Eye.Temp, Treatment, Sex, Timeline,
    Hour, Amb.Temp, Direction, Pen, Bird_Scramble,
    Date.of.Photo, Treat_Day
...
# No obvious trend. Checking for normality
...
# Appears normal. Checking for patterns across predictors.

{
  p1 <- Acute_Full_DS$data %>%
    dplyr::select(
      Maximum.Eye.Temp, Treatment, Sex, Timeline, 
      Hour, Amb.Temp, Direction, Pen, Bird_Scramble,
      Date.of.Photo, Treat_Day
...
# Assessing condition trends.

conditional_smooths(Acute_Full_DS) # Treatment by ambient temperature interaction weak, but reasonable perameterisation. 

# Good. Re-executing non-scrambled (or "true") model, then comparing repeatability measures.
...
S_Rep_Acute_Base <- Rep_Treats_Acute_Base %>% ggplot(aes(x = T_Repeatability, fill = Mod_Type)) +
...
C_Rep_Acute_Base <- Rep_Treats_Acute_Base %>% ggplot(aes(x = C_Repeatability, fill = Mod_Type)) +
  geom_density(alpha = 0.5, adjust = 5) +
  xlim(0, 1) +
  scale_fill_manual(values = c("black", "firebrick4"), name = NULL) + 
  scale_y_continuous(trans = "sqrt", name = "Posterior Density") +
...
grid.arrange(S_Rep_Acute_Base, C_Rep_Acute_Base, nrow = 2)

# Control slopes are strikingly similar, but some detectable divergence in stress-exposed slopes.

require(showtext)

ggsave("/home/joshk/git_repositories/BCCH_IndVar/Figures/Acute_Repeat_DS_Base.jpeg",
  S_Rep_Acute_Base,
...
ggsave("/home/joshk/git_repositories/BCCH_IndVar/Figures/Acute_Repeat_DS_Base.pdf",
       S_Rep_Acute_Base,
       height = 6.0, width = 6.0, dpi = 800
)
...
Wide_Acute_Rep <- Rep_Treats_Acute_Base %>%
...
# Assessing repeatability of acute changes in heat transfer
...
ggsave("/home/joshk/git_repositories/BCCH_IndVar/Figures/Acute_Repeat_DS_New.jpeg",
  S_Rep_Acute,
  height = 6.0, width = 6.0, dpi = 800
)
...
ggsave("/home/joshk/git_repositories/BCCH_IndVar/Figures/Acute_Repeat_DS_New.pdf",
       S_Rep_Acute,
       height = 6.0, width = 6.0, dpi = 800
)
...
stan_ac(CC_Full_DS$fit) # Autocorrelation low
mcmc_neff(neff_ratio(CC_Full_DS), size = 2)
mcmc_rhat(rhat(CC_Full_DS))

# Ratio of effective sample size to sample size is fair (> 0.7). Rhat values great.
...
CC_Full_DS$data %>%
  ...
  na.omit(.) %>%
  add_predicted_draws(CC_Full_DS) %>%
  group_by(
    Maximum.Eye.Temp, Treatment, Sex,
    Hour, s.Amb.Temp, Direction, Pen, Bird_Scramble,
    Date.of.Photo, Treat_Day
...
# Again, good. Checking residuals across predictors.
...
# Zooming in on bird identities

print(p7) # Again, SEs of most cross zero, and others draw close.

# Visualising and summarising trends

conditional_smooths(CC_Full_DS)
summary(CC_Full_DS)

# Running original model then calculating and comparing repeatabilities
...
S_Rep_Chronic <- Rep_Treats %>% ggplot(aes(x = T_Repeatability, fill = Mod_Type)) +
  geom_density(alpha = 0.5, adjust = 5) +
  xlim(0, 1) +
  #scale_fill_viridis_d(begin = 0.3, end = 0.7, name = NULL) +
  scale_fill_manual(values = c("black", "firebrick4"), name = NULL) + 
...
C_Rep_Chronic <- Rep_Treats %>% ggplot(aes(x = C_Repeatability, fill = Mod_Type)) +
  geom_density(alpha = 0.5, adjust = 5) +
  xlim(0, 1) +
  scale_fill_manual(values = c("black", "firebrick4"), name = NULL) + 
  scale_y_continuous(trans = "sqrt", name = "Posterior Density") +
...
grid.arrange(S_Rep_Chronic, C_Rep_Chronic, nrow = 2)

# Subtle increases among true models, but little. Saving plot then runnning formal comparison

require(showtext)

ggsave("/home/joshk/git_repositories/BCCH_IndVar/Figures/Chronic_Repeat_DS_New.jpeg",
  S_Rep_Chronic,
  height = 6.0, width = 6.0, dpi = 800
)
...
ggsave("/home/joshk/git_repositories/BCCH_IndVar/Figures/Chronic_Repeat_DS_New.pdf",
       S_Rep_Chronic,
       height = 6.0, width = 6.0, dpi = 800
)
...
stan_ac(CC_HT_DS$fit) # No clear autocorrelation
mcmc_neff(neff_ratio(CC_HT_DS), size = 2) # Neff/N > 0.6
mcmc_rhat(rhat(CC_HT_DS)) # Rhats all close to 1.

# Checking residuals by fitteds 
...
# Reasonably normal. Checking for heteroskedasticity
...
# Again, zooming in on bird identities

print(p7) # Great.

# Visualising and summarising model results

conditional_smooths(CC_HT_DS)
summary(CC_HT_DS)

# Re-unning original model to calculate and compare repeatability estimates
...
# Similar patterns to surface temperature
...
# Yes, rather similat. Saving plot and runnning formal comparison

require(showtext)

ggsave("/home/joshk/git_repositories/BCCH_IndVar/Figures/Chronic_HT_Repeat_DS_New.jpeg",
  S_Rep_Chronic,
  height = 6.0, width = 6.0, dpi = 800
)
...
ggsave("/home/joshk/git_repositories/BCCH_IndVar/Figures/Chronic_HT_Repeat_DS_New.pdf",
       S_Rep_Chronic,
       height = 6.0, width = 6.0, dpi = 800
)
